...
from numpy import inf, array, finfo, cos, sin, arctan2
...
def anisotropic_plate(Rx,Ry,S,thickness):
    ...
    if abs(S[5,0]) < meps and abs(S[5,1]) < meps and abs(S[1,1] - S[0,0]) < meps and abs(S[0,0] + S[1,1] - 2*S[0,1] - S[5,5]) < meps:
        ...
        alpha = 0.5*arctan2(Da*(invRy+invRx) - Ba*(invRy-invRx), Aa*(invRy-invRx) - Ca*(invRy+invRx))

    #rotate S by alpha
    Sp = rotate_elastic_matrix(S, 'S', inplane_rotation(alpha))

    #Precomputed coefficients
    mx = 0.5*((Sp[0,1]-Sp[1,1])*(invRy + invRx) + (Sp[0,1]+Sp[1,1])*(invRy - invRx)*cos(2*alpha))/(Sp[0,0]*Sp[1,1] - Sp[0,1]*Sp[0,1])
    my = 0.5*((Sp[0,1]-Sp[0,0])*(invRy + invRx) + (Sp[0,1]+Sp[0,0])*(invRy - invRx)*cos(2*alpha))/(Sp[0,0]*Sp[1,1] - Sp[0,1]*Sp[0,1])

    coef1 = Sp[2,0]*mx + Sp[2,1]*my
    coef2 = (Sp[4,0]*mx + Sp[4,1]*my)*cos(alpha) - (Sp[3,0]*mx + Sp[3,1]*my)*sin(alpha)
